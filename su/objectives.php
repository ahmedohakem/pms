...
FROM courses cr, objectives_2020 o, obj_trn_months_2020 trm, months m
...
{
$SumWeightsAndScoresQur = mysqli_query($conn, "SELECT SUM(obj_weight) AS WeightsSum FROM objectives_2020 WHERE obj_year = '2020' AND employee_id = '".$employee_id."'");
...
?>
<?php
$SelectQur = mysqli_query($conn, "SELECT employee_id FROM objectives_submission WHERE employee_id = '".$employee_id."' AND objectives_year = '2020' AND filed = '0'");
if(mysqli_num_rows($SelectQur) == 0) {
//
}
else
{
?>
<form method='POST' action='next_cycle_objectives_filed_proc.php'><input type='hidden' name='employee_id' value='<?php echo $employee_id; ?>'><input type='hidden' name='company_id' value='<?php echo $_POST['company_id']; ?>'><input type='submit' onclick='return confirm("are you sure this form signed and recieved");'value='Confirm the Signed Hard Copy Recived'></form>
<?php
}
?>
<form method='POST' action='objectives_report.php'><input type='hidden' name='company_id' value='<?php echo $_POST['company_id']; ?>'><input type='submit' value='Select Another Employee'></form>
