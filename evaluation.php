...
{
header("location:evaluation_fc.php?employee_id=".encrypt_decrypt('encrypt', $employee_id));
die();
}
include('includefiles/employee_info.php');
include('includefiles/anti_hack.php');
include('includefiles/competencies_data_space_check.php');
?>
...
if($ProcStatus == 2){
$ProcStatusText = "<font color='red'>This form not ready to send (check OBJECTIVES)</font>";
}
if($ProcStatus == 3){
$ProcStatusText = "<font color='red'>This form not ready to send (check COMPETENCIES)</font>";
}
}
else
{
...
<p>&nbsp;</p>
<form method='POST' action='evaluation_save_changes.php'>
<?php
...
while($row = mysqli_fetch_array($SelectQur))
  ...
  echo "<td  align= 'center'><select name='".$row['obj_id']."'>
      <option selected value='".$row['obj_rating']."'>".$row['obj_rating']."</option>
...
while($row = mysqli_fetch_array($SelectQur))
  ...
  echo "<td  align= 'center'><select name='".$row['comp_id']."'>
      <option selected value='".$row['comp_rating']."'> ".$row['comp_rating']." </option>
      <option value='1'>1</option>
      <option value='2'>2</option>
      <option value='3'>3</option>
      <option value='4'>4</option>
